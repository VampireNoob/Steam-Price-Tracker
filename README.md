...
- **Web frontend** (`app.py`, Flask) — a card-based browser view of the tracked games with thumbnails, current price/discount, and last-checked timestamp. Includes a live search filter, a light/dark theme toggle (persisted via `localStorage`), search and add new games (with preview images), remove games, and a manual price-check button, all from `http://127.0.0.1:5000`.
