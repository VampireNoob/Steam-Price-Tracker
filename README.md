...
A small console application built as a school project to practice Python fundamentals: connecting to an external API, persisting data, and sending notifications. It tracks the Steam prices of a user-defined list of games and sends a Telegram message whenever a price changes — and can now be controlled directly from Telegram, not just the terminal.
...
- **Telegram bot control** (`bot_listener.py`) — runs continuously (long polling) and reacts to commands sent directly in the Telegram chat: `/list`, `/add <name>`, `/remove <number>`, `/check` (runs a price check on demand), `/help`. Only messages from the configured chat ID are processed.
- **Automation** — both scripts run unattended via Windows Task Scheduler: `price_tracker.py` daily at a fixed time, `bot_listener.py` at login (via `pythonw.exe`, no visible window), so notifications and bot commands work without manually starting anything.
- **Shared logic** (`common.py`) — configuration loading, `games.json` read/write, Telegram sending, and Steam search are factored out so all three scripts use the same code instead of duplicating it.

All of the above has been manually tested end to end, including simulated and real price changes (e.g. a live Steam sale), Telegram bot commands, and both scheduled tasks running unattended.
...
5. `python manage_games.py` — add the games you want to track (or use `/add <name>` via `bot_listener.py` once it's running)
...
7. `python bot_listener.py` — starts the interactive bot control; runs until stopped (`Ctrl+C`)

> `games.json` is created automatically the first time you add a game via `manage_games.py` (or `/add` via Telegram). `games.example.json` just shows the expected format.
...
- **Web frontend** — a browser-based interface (planned with Flask) as an alternative to the console/Telegram controls.
...
These are intentionally out of scope for the current version — everything above runs locally and depends on the PC being on.

## Tech stack

Python, `requests` (HTTP), `python-dotenv` (config), SQLite (price history), Steam Store API (search + price data), Telegram Bot API (notifications + interactive commands), Windows Task Scheduler (automation), Python `logging` (rotating file logs).
